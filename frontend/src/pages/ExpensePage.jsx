...
const IncomePage = ({ theme }) => {
  ...
  const totalExpense = expenseTransactions.reduce((acc, t) => acc + t.amount, 0).toFixed(2);
  ...
  if (loading) return <div className="p-8"><p>Loading Expense Data...</p></div>;
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className={`${theme.glass} border ${theme.cardBorder} rounded-2xl p-6 mb-6`}>
        <div className="flex items-center justify-between mb-2">
          <span className={`text-sm font-semibold uppercase ${theme.mutedText} tracking-wider`}>Total Expense</span>
          <TrendingDown className="text-red-400" size={24} />
        </div>
        <p className="text-4xl font-bold text-red-400">+${totalExpense}</p>
      </div>
